...
--修改表的数据存储位置
--load data inpath '${hiveconf:DATA_PATH}' overwrite into table sunny.vehicle_trajectory;
...
--停留时间top n
create table if not exists sunny.staytime_top100 as 
SELECT /*+ MAPJOIN(b) */ b.datetime,b.terminalid,b.time,b.lat,b.lon,b.direction,b.speed,(coalesce(cast(a.time as bigint) - cast(b.time as bigint))) sytime 
from 
    sunny.vehicle_trajectory_tmp b left outer join  sunny.vehicle_trajectory_tmp a
on 
    (a.terminalId=b.terminalId 
and 
    a.row_num = b.row_num+1) order by sytime desc limit 100;
...
select /*+ MAPJOIN(po) */ rs.datetime,rs.terminalid,rs.sytime,rs.p_name,rs.p_code,rs.lon,rs.lat from (
    select  po.datetime,po.terminalid,po.sytime,go.area_name p_name,go.area_code p_code,po.lon,po.lat ,st_intersects(st_point(po.lon,po.lat), st_polygon(go.lnlts)) flag 
from (
    select datetime,terminalid,time,lon,lat,direction,speed,sytime 
    from sunny.staytime_top100) po join
(
...
select /*+ MAPJOIN(pro) */ city.area_name c_name,city.area_code c_code,pro.lon,pro.lat,
...
on city.province_code = pro.p_code ) cp
where cp.flag=true;
...
select /*+ MAPJOIN(poc) */  dt.area_name d_name,dt.area_code d_code,poc.lon,poc.lat,
...
select sq.b_name,sq.b_code,sq.b_type,sq.lon,sq.lat,sq.business_geo from(
select /*+ MAPJOIN(pcd) */   bn.area_name b_name,bn.area_code b_code,bn.business_type b_type,pcd.lon,pcd.lat, 
st_intersects(st_point(pcd.lon,pcd.lat),st_polygon(cast(bn.lnlts as string))) flag,bn.business_geo
from 
(select d_name,d_code,lon,lat from sunny.district_geo_tmp) pcd
left outer join
(select area_name,business_type,area_code,district_code,concat("polygon((",regexp_replace(regexp_replace(business_geo,","," "),"\;",","),"))") lnlts,business_geo from sunny.map_gd_business_tmp) bn 
 on pcd.d_code=bn.district_code
...
--多表联合
create table if not exists sunny.trajectory_cloud  as select datetime,terminalid,second2hm(sytime) sytime,p_name,p_code,c_name,c_code,d_name,d_code, b_name,b_code,b_type,cy.lon,cy.lat,business_geo from 
(select /*+ MAPJOIN(pn,cy) */  datetime,terminalid,sytime,p_name,p_code,lon,lat from sunny.province_geo_tmp) pn 
left outer join 
...
left outer join 
(select b_name,b_code,b_type,lon,lat,business_geo from sunny.business_geo_tmp) bn
...
--导出trajectory_cloud
insert overwrite local directory '/opt/sunny/hive-export/original/' row format delimited fields  terminated by '\t' select datetime,terminalid,sytime,p_name,p_code,c_name,c_code,d_name,d_code, b_name,b_code,b_type,lon,lat,business_geo from sunny.trajectory_cloud;
